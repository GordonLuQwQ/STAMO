...
# from STAMO.stamo.renderer.model.renderersd3 import RenderNet
# from STAMO.stamo.renderer.trainer_1 import Trainer
...
def main_worker(args):
    if bool(getattr(args, "deepspeed", False)):
        raise ValueError(
            "validate_renderer.py loads a portable renderer export; run it "
            "without --deepspeed and point resume_path at renderer_exports/<step>."
        )
    # configs/flux.yaml is a training config by default.  Keep its LoRA model
    # structure so the portable adapter can load, but disable optimizer/DS
    # requirements and dataset fingerprinting in this evaluation process.
    args.do_train = False
    args.train.iter_per_ep = None
    overwatch.info("Building models...")
    model = RenderNet(args)
