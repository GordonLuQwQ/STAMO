...
export TORCH_MCCL_BLOCKING_WAIT="${TORCH_MCCL_BLOCKING_WAIT:-1}"

if [[ "${DS_ACCELERATOR,,}" != "musa" ]]; then
    echo "FAIL: DS_ACCELERATOR must be musa, got ${DS_ACCELERATOR}" >&2
    exit 2
fi
if ! [[ "${TRAIN_STEPS}" =~ ^[0-9]+$ ]] || (( TRAIN_STEPS < 100 )); then
    echo "FAIL: TRAIN_STEPS must be an integer >= 100 (200 is recommended)." >&2
    exit 2
fi
...
if [[ "${TORCH_MCCL_BLOCKING_WAIT}" != "1" ]]; then
    echo "FAIL: TORCH_MCCL_BLOCKING_WAIT must be 1 for the MUSA admission test." >&2
    exit 2
fi
...

{
    ...
    echo "TORCH_MCCL_BLOCKING_WAIT=${TORCH_MCCL_BLOCKING_WAIT}"
...
# Clipboard/editor deployments can accidentally append the replacement file
# to the old source. Python will then silently use the last class definition,
# so an import-only probe is insufficient. Reject concatenated source before
# allocating model memory or starting any collective.
trainer_source = expected_trainer.read_text(encoding="utf-8")
trainer_tree = ast.parse(trainer_source, filename=str(expected_trainer))
required_unique_definitions = {
    "Trainer": ast.ClassDef,
    "_ensure_deepspeed_zero3_ipg_bucket_reset_compatibility": ast.FunctionDef,
    "_assert_deepspeed_zero3_ipg_buckets_cleared": ast.FunctionDef,
}
for definition_name, definition_type in required_unique_definitions.items():
    count = sum(
        isinstance(node, definition_type) and node.name == definition_name
        for node in trainer_tree.body
    )
    if count != 1:
        raise RuntimeError(
            f"trainer.py must contain exactly one top-level {definition_name} "
            f"definition, found {count}; the file may have been appended"
        )

entrypoint_source = (repo_root / "train_renderer.py").read_text(encoding="utf-8")
entrypoint_tree = ast.parse(
    entrypoint_source,
    filename=str(repo_root / "train_renderer.py"),
)
main_count = sum(
...
if main_count != 1:
    raise RuntimeError(
        "train_renderer.py must contain exactly one top-level main definition, "
        f"found {main_count}; the file may have been appended"
    )
...
print(f"TORCH_MCCL_BLOCKING_WAIT={os.environ.get('TORCH_MCCL_BLOCKING_WAIT')}")
...
echo "  8-rank exact-shape MCCL: 9437184 elements x 100 iterations"
...
echo "  MUSA MCCL blocking wait: enabled"
